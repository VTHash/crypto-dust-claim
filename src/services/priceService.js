...
export async function getTokenUsdPrices(chainId, addresses = []) {
  ...
  if (!platform || !addresses.length) {
    if (!platform) {
      console.warn(
        `getTokenUsdPrices: no platform mapping for chainId=${chainId}`
      )
    }
    return {}
  }
  ...
  try {
    ...
    console.error(
      `Error fetching token prices for chain ${chainId}:`,
      e.message
    )
    return {}
  }
...
export async function getMultipleNativePrices(chainIds = []) {
  const coinIds = [
    ...new Set(
      chainIds.map((id) => NATIVE_ID_BY_CHAIN[id]).filter(Boolean)
    )
  ]
...
export async function getHistoricalPrice(chainId, address, days = 7) {
  ...
  try {
    ...
    console.error(
      `Error fetching historical price for ${address}:`,
      e.message
    )
    return null
  }
...
export async function getTokenMetadataAndPrice(chainId, address) {
  ...
  try {
    ...
    console.error(
      `Error fetching token metadata for ${address}:`,
      e.message
    )
    return null
  }
